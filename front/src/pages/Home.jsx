...
const Home = () => {
    ...
    const archiveInvoice = async (id) => {
        ...
        try {
            await axios.get('http://localhost:3333/api/invoices/archive/' + id);
            window.location.reload();
    ...
    const seeInvoice = async (id) => {
        try {
            const response = await axios.get('http://localhost:3333/api/invoices/' + id);
            //  redirect to home
            window.open('http://localhost:3333/' + response.data.image_url, '_blank');
