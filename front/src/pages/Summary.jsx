	...
	const fetchSummaries = async () => {
		try {
			const response = await axios.get(
				"http://52.90.40.101:3333/api/summaries"
			);

			let datatest = response.data.map((obj) => {
				return [
					new Date(obj.start_date).toLocaleDateString() +
						" - " +
						new Date(obj.end_date).toLocaleDateString(),
					new Date(obj.generated_date).toLocaleDateString(),
					<>
						<span
							onClick={() => seeSummary(obj.pdf_link)}
							className="material-icons cursor-pointer"
						>
							download
						</span>
						<span
							onClick={() => deleteSummary(obj._id)}
							className="material-icons cursor-pointer"
						>
							delete
						</span>
		...
		<main className="page">
			...
			<div className="right-container">
				...
				<div className="bg-white flex flex-col justify-items-center items-center p-5 rounded-md">
					...
					<button
						onClick={generateLastSixMonthsSummary}
						className="btn-black my-3"
						style={{ width: "200px" }}
					>
						Les 6 derniers mois
					</button>
			...
			{isModalOpen && (
				<div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
					<div className="bg-white rounded-lg shadow-lg lg:w-full max-w-lg p-6">
